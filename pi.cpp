...
const lli inv_2 = inv(2, Mod), inv_6 = inv(6, Mod), inv_30 = inv(30, Mod);
...
lli sum(lli n, int k){
	...
	if(k == 3) return pow(n * (n + 1) % Mod * inv_2 % Mod, 2, Mod);
...
lli SumPrimePi(lli n, int k){
	...
	for(p = 2; p <= v; p++){
		...
		temp = lo[p-1];
		q = p * p;
		hi[1] -= (hi[p] - temp) * pow(p, k, Mod) % Mod;
		if(hi[1] < 0) hi[1] += Mod;
		j = 1 + (p & 1);
		end = (v <= n/q) ? v : n/q;
		for(i = p + j; i <= 1 + end; i += j){
			...
			d = i * p;
			if(d <= v)
				hi[i] -= (hi[d] - temp) * pow(p, k, Mod) % Mod;
			else
				hi[i] -= (lo[n/d] - temp) * pow(p, k, Mod) % Mod;
			if(hi[i] < 0) hi[i] += Mod;
		}
		if(q <= v)
			for(i = q; i <= end; i += p*j)
				used[i] = true;
		for(i = v; i >= q; i--){
			lo[i] -= (lo[i/p] - temp) * pow(p, k, Mod) % Mod;
			if(lo[i] < 0) lo[i] += Mod;
		}
	}
	return hi[1] % Mod;
