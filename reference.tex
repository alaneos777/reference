...
\begin{document}
	...
	\section{Teoría de números}
		...
		\subsection{Otros}
			...
			\subsubsection{Números de Stirling del segundo tipo}
	...
	\section{FFT}
		...
		\inputminted[tabsize=2,breaklines,firstline=36,lastline=75,fontsize=\small]{c++}{fft.cpp}
		...
		\inputminted[tabsize=2,breaklines,firstline=77,lastline=99,fontsize=\small]{c++}{fft.cpp}
		
		\subsection{Aplicaciones}
			\subsubsection{Multiplicación de números enteros grandes}
			\inputminted[tabsize=2,breaklines,firstline=101,lastline=134,fontsize=\small]{c++}{fft.cpp}
			
			\subsubsection{Recíproco de un polinomio}
			\inputminted[tabsize=2,breaklines,firstline=136,lastline=160,fontsize=\small]{c++}{fft.cpp}
			
			\subsubsection{Raíz cuadrada de un polinomio}
			\inputminted[tabsize=2,breaklines,firstline=162,lastline=184,fontsize=\small]{c++}{fft.cpp}
			
			\subsubsection{Cociente y residuo de dos polinomios}
			\inputminted[tabsize=2,breaklines,firstline=186,lastline=211,fontsize=\small]{c++}{fft.cpp}
			
			\subsubsection{Multievaluación rápida}
			\inputminted[tabsize=2,breaklines,firstline=213,lastline=257,fontsize=\small]{c++}{fft.cpp}
			
			\subsubsection{DFT con tamaño de vector arbitrario (algoritmo de Bluestein)}
			\inputminted[tabsize=2,breaklines,firstline=259,lastline=279,fontsize=\small]{c++}{fft.cpp}
			
		\subsection{Convolución de dos vectores reales con solo dos FFT's}
		\inputminted[tabsize=2,breaklines,firstline=281,lastline=301,fontsize=\small]{c++}{fft.cpp}
			
		\subsection{Convolución con módulo arbitrario}
		\inputminted[tabsize=2,breaklines,firstline=303,lastline=362,fontsize=\small]{c++}{fft.cpp}
	...
	\section{Fórmulas y notas}
		\begin{itemize}
			\item \textbf{Números de Stirling del primer tipo (sin signo):} $\stirlingI{n}{k}$ representa el número de permutaciones de $n$ elementos en exactamente $k$ ciclos disjuntos.
			\begin{align*}
				\stirlingI{0}{0} &= 1 \\
			...
			\item \textbf{Números de Stirling del segundo tipo:} $\stirlingII{n}{k}$ representa el número de formas de particionar un conjunto de $n$ objetos distinguibles en $k$ subconjuntos no vacíos.
			\begin{align*}
				\stirlingII{0}{0} &= 1 \\
			...
			\item \textbf{Números de Bernoulli:}
			\begin{align*}
				{B_0}^+ &= 1 \\
			...
			\item \textbf{Fórmula de Faulhaber:}
			\begin{align*}
			...
			\item \textbf{Funciones generadoras:}
			\begin{align*}
			...
			\item \textbf{Árbol de Stern–Brocot:} todos los racionales positivos se pueden representar como un árbol binario de búsqueda completo infinito con raíz $\frac{1}{1}$.
			\begin{itemize}
				...
				\item Para hallar el camino de la raíz $\frac{1}{1}$ a un racional $q$, se usa búsqueda binaria iniciando con $L=\frac{0}{1}$ y $R=\frac{1}{0}$. Para hallar $M$ se supone que $L=\frac{a}{b}$ y $\frac{c}{d}$, entonces $M=\frac{a+c}{b+d}$.
			\end{itemize}
			...
			\begin{itemize}
				...
				\item Para un grafo no dirigido simple $G$ con $n$ vértices etiquetados de $1$ a $n$, sea $Q=D-A$, donde $D$ es la matriz diagonal de los grados de cada nodo de $G$ y $A$ es la matriz de adyacencia de $G$. Entonces el número de spanning trees de $G$ es igual a cualquier cofactor de $G$.
			\end{itemize}
			...
			\item \textbf{Primos:} $10^2+1$, $10^3+9$, $10^4+7$, $10^5+3$, $10^6+3$, $10^7+19$, $10^8+7$, $10^9+7$, $10^{10}+19$, $10^{11}+3$, $10^{12}+39$, $10^{13}+37$, $10^{14}+31$, $10^{15}+37$, $10^{16}+61$, $10^{17}+3$, $10^{18}+3$
		\end{itemize}
